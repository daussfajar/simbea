...
class Datatables
{
    ...
    private $limit_total_results = null;
    private $get_num_rows = false;
    ...
    // limit total result
    public function limit_total_result($number)
    {
        $this->limit_total_results = $number;
        return $this;
    }

    // get num rows (untuk menghindari error duplicate cloumn)
    public function get_num_rows()
    {
        $this->get_num_rows = true;
        return $this;
    }
    ...
    private function get_total_results($filtering = FALSE)
    {
        ...
        if ($this->distinct != null && strlen($this->distinct) > 0) {
        ...
        // check using limit total results or not
        if ($this->limit_total_results != null) {
            $this->ci->db->limit($this->limit_total_results);
        }

        if ($this->get_num_rows == true) {
            return $this->ci->db->get()->num_rows();
        } else {
            return $this->ci->db->count_all_results();
        }
        if ($this->having == true) {
            return $this->ci->db->having();
        }
    }
